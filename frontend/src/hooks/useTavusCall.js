...
/**
 * Call our own backend tool endpoint and return the parsed result.
 */
async function fetchToolResult(toolName, conversationId, args) {
  ...
  switch (toolName) {
    ...
    case 'check_in_patient':
      if (result.status === 'checked_in') {
        return `TOOL_RESULT check_in_patient: SUCCESS. Patient is now checked in. Tell them to have a seat.`;
      }
      if (result.status === 'already_checked_in') {
        return `TOOL_RESULT check_in_patient: ALREADY_CHECKED_IN. Patient was already checked in.`;
      }
      return `TOOL_RESULT check_in_patient: ERROR. ${result.message || 'Check-in failed.'}`;
    ...
    case 'book_appointment':
      if (result.status === 'success') {
        return `TOOL_RESULT book_appointment: SUCCESS. ${result.message} Confirmation: ${result.confirmation_number}.`;
      }
      return `TOOL_RESULT book_appointment: ERROR. ${result.message || 'Booking failed.'}`;

    case 'send_sms_reminder':
      if (result.status === 'success') {
        return `TOOL_RESULT send_sms_reminder: SUCCESS. ${result.message}`;
      }
      return `TOOL_RESULT send_sms_reminder: ERROR. ${result.message || 'SMS failed.'}`;

    default:
      return `TOOL_RESULT ${toolName}: ${JSON.stringify(result)}`;
  }
}
